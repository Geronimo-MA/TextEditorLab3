...
```text
Vn = { Z, <тернарное-выражение>, <условие>, <операнд>, <оператор-отношения> }
```
...
2) <тернарное-выражение> -> <условие> sp ? sp <операнд> sp : sp <операнд>

3) <условие> -> <операнд> sp <оператор-отношения> sp <операнд>

4) <операнд> -> id | num

5) <оператор-отношения> -> > | < | >= | <= | == | !=
```
...
### Иллюстрация
![Классификация грамматики](images/grammar_classification.png)
...
![Диаграмма рекурсивного спуска](images/recursive_descent_diagram.png)
...
В качестве точек синхронизации используются ключевые элементы конструкции тернарного оператора:
- `?`
- `:`
- `;`
